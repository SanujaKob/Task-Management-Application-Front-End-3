...
import { listAllTasks, updateTask, deleteTask } from '@/services/tasks' // <- all users
...
async function load() {
  ...
  error.value = ''
  try {
    const res = await listAllTasks() // <- pulls every task
    const rows = Array.isArray(res) ? res : (res?.items ?? [])
    // optional client-side filters to mirror MyTasks
    let filtered = rows
    if (q.value.trim()) {
      ...
      filtered = filtered.filter(t =>
        ...
        String(t.assignee_name ?? '').toLowerCase().includes(needle)
      )
    }
    if (statusFilter.value) {
      const api = STATUS_LABEL_TO_API[statusFilter.value]
      filtered = filtered.filter(t => t.status === api)
    }
...
onMounted(load)
let deb; watch([q, statusFilter, overdueOnly], () => { clearTimeout(deb); deb = setTimeout(load, 250) })
...
<template>
  <div class="page">
    <!-- Centered hero -->
    <section class="hero">
      <h1 class="hero__title">Team Tasks</h1>
      <p class="hero__subtitle">
        All users’ tasks in one place. Review, update, and keep the team moving.
      </p>
    </section>
    ...
    <section class="toolbar">
      <v-text-field
        v-model="q" label="Search (id / title / desc / assignee)" prepend-inner-icon="mdi-magnify"
    ...
    <div v-if="loading" class="block">
      <v-skeleton-loader type="article, card, list-item-two-line, list-item-two-line" loading />
    </div>
    ...
    <div v-else-if="sortedFiltered.length === 0" class="empty-wrap">
      <v-card class="empty-card" variant="tonal">
        <v-card-text class="empty-text">
          ...
          <div class="empty-title">No team tasks to show</div>
    ...
    <div v-else class="grid">
      ...
      >
        <v-card-item>
          ...
          <div class="task-meta">
            <span>Assignee: <b>{{ t.assignee_name ?? (t.assignee_id ? 'User #' + t.assignee_id : '—') }}</b></span>
